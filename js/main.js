...
function update_vhosts() {
    ...
    // 去除All vhost选项
    select.options.length = vhosts.length;
    var index = 0;
    for (var i = 0; i < vhosts.length; i++) {
        var vhost = vhosts[i].name;
        select.options[i] = new Option(vhost, vhost);
        if (vhost == current_vhost) index = i;
    }
    select.selectedIndex = index;
...
function with_req(method, path, body, fun) {
    ...
    req.open(method, 'http://' + serverUrl + ':15673/api' + path, true);
...
function sync_req(type, params0, path_template, options) {
    ...
    var req = xmlHttpRequest();
    req.open(type, 'http://' + serverUrl + ':15673/api' + path, false);
    req.setRequestHeader('x-vhost', current_vhost);
    req.setRequestHeader('tenant', current_tenant);
    req.setRequestHeader('content-type', 'application/json');
